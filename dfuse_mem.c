...
struct memsegment *find_segment(struct memsegment *segment_list,
  ...
  while (segment_list != NULL) {
    if (segment_list->start <= address && segment_list->end >= address)
      return segment_list;
...
struct memsegment *parse_memory_layout(char *intf_desc) {
  ...
  while (ret = sscanf(intf_desc, "/0x%x/%n", &address, &scanned), ret > 0) {

    intf_desc += scanned;
    while (ret = sscanf(intf_desc, "%d*%d%c%[^,/]%n", &sectors, &size,
                        &multiplier, typestring, &scanned),
           ret > 2) {
      intf_desc += scanned;
      ...
      memtype = 0;
      if (ret == 4) {
        if (strlen(typestring) == 1 && typestring[0] != '/')
